...
本软件的主要功能是根据输入的照度值，选择某一固定的 ISO 感光度、光圈数、快门值，计算出对应的曝光参数组合。

在软件中，用户可以选择不同的 ISO 感光度、光圈数、快门值，并通过调整光圈大小，以便在不同光圈下计算出最佳的曝光参数。这样，摄影师可以更加专注于构图和创作，而不必过多关注曝光参数的调整。
...
首先，您需要访问[本项目的release页面](https://github.com/EasyCam/LUX2EV_beeware/tree/main/release)。
在该页面中选择适合您的语言版本的压缩包。如果您需要中文版本，请选择带有"cn"字样的压缩包；如果您需要英文版本，请选择带有"en"字样的压缩包。
